...
Combinaison detecterSuiteX(int grille[NB_LIGNES][NB_COLONNES],int taille)
{
	...
	for (int i=0; i<NB_LIGNES; i++){
		...
		while (j<NB_COLONNES) {
			...
			while (case_suivante < NB_COLONNES && (grille[i][case_suivante] == case_checker || grille[i][case_suivante] == JOKER || case_checker == JOKER)) {
				if (case_checker == JOKER && grille[i][case_suivante] != JOKER) {
					case_checker = grille[i][case_suivante];
				}
				case_suivante++;
			}
...
Combinaison detecterSuiteY(int grille[NB_LIGNES][NB_COLONNES], int taille) {
	...
	for(int i =0; i<NB_COLONNES;i++){
		for (int j=0; j<NB_LIGNES; j++) {
			int case_checker = grille[j][i];
			int case_suivante = j+1;
			while (case_suivante <NB_LIGNES && (grille[case_suivante][i] == case_checker || grille[case_suivante][i] == JOKER || case_checker == JOKER)) {
				if (case_checker == JOKER && grille[case_suivante][i] != JOKER) {
					case_checker = grille[case_suivante][i];
				}
				case_suivante += 1;
			}
			int longeur_de_suite = case_suivante-j;
